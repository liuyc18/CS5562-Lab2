...
def ep_train(poisoned_train_data_path, trigger_ind, ori_norm, model, parallel_model, tokenizer, 
            batch_size, epochs, lr, criterion, device, seed, save_model=True, save_path=None):
    print('Seed: ' + str(seed))
    random.seed(seed)
    np.random.seed(seed)
    ...
    for epoch in range(epochs):
        ...
        model.train(True)
        model, poison_train_loss, poison_train_acc = ep_train_epoch(trigger_ind, ori_norm, model, 
                                                                    parallel_model, tokenizer,
